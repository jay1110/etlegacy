...
// 4. The standalone cgame/ui side modules must be present in legacy/.
const sideModules = ['cgame.mp.wasm32.so', 'ui.mp.wasm32.so'];
for (const so of sideModules) {
    check(exists(path.join('legacy', so)), `side module present: legacy/${so}`);
}
...
if (pk3) {
    try {
        ...
        const listing = execFileSync('unzip', ['-l', path.join(legacyDir, pk3)], {
            encoding: 'utf8'
        });
        for (const so of sideModules) {
            check(listing.includes(so), `mod pk3 contains ${so}`);
        }
    } catch (e) {
