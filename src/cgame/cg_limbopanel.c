...
{
	CG_DrawMap(button->rect.x, button->rect.y, button->rect.w, button->rect.h, cgs.ccFilter, NULL, qtrue, 1.f, qtrue, qtrue);
	...
	else if (button->data[0] == 4)  // skill count
	{
		if (toolTipSkill != SK_NUM_SKILLS)
		{
			...
			CG_LimboPanel_DrawToolTips(&toolTipRect, &objectivePanelTxt, colorWhite,
			                           va("%s%s\n%sLevel %i: %s\n^*%s", GetSkillTableData(toolTipSkill)->skillNames, isSkillAvailable ? "" : " ^1[SKILL LEVEL DISABLED]",
