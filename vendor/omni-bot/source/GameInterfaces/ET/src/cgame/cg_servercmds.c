...
qboolean CG_AddOnScreenText( const char *text, vec3_t origin, float duration );
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "wp" ) ) 
	{
		CG_AddOnScreenText(CG_Argv(1), tv(atof(CG_Argv(2)), atof(CG_Argv(3)), atof(CG_Argv(4))), atof(CG_Argv(5)));
		return;
	}
