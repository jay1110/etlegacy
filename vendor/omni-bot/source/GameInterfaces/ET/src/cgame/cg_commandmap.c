...
//	vec4_t c_clr = {1.f, 1.f, 1.f, 1.f};
	...
	for(i = 0, mEnt = &mapEntities[0]; i < mapEntityCount; i++, mEnt++ ) {
		if (mEnt->team != CG_LimboPanel_GetRealTeam() && cgs.clientinfo[cg.clientNum].team != TEAM_SPECTATOR) {
			continue;
		}
		...
		if( changetime ) {
			...
			if( draw ) {
				...
				if( !scissor ) {
					...
					Com_sprintf( buffer, sizeof( buffer ), "%s (^3%i^7) (Troops: %i)", cg.spawnPoints[i], i, cg.spawnPlayerCounts[i] );
